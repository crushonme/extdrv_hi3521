...
{
	//HW Reset
	gpio_i2c_write(0x72,0x82,0x25);
	gpio_i2c_write(0x72,0x08,0x36);
	gpio_i2c_write(0x72,0x7c,0x14);
	gpio_i2c_write(0x72,0xc7,0x00);
	DelayMS(100);

	//9022 PLL Set:
	gpio_i2c_write(0x72,0xBC,0x01);
	gpio_i2c_write(0x72,0xBD,0x82);
	gpio_i2c_write(0x72,0xBE,0x35);

	//9022A PLL Set
	gpio_i2c_write(0x72,0x08,0x70);

	gpio_i2c_write(0x72,0x00,0x8C);
	gpio_i2c_write(0x72,0x01,0x0A);
	gpio_i2c_write(0x72,0x02,0x70);
	gpio_i2c_write(0x72,0x03,0x17);
	gpio_i2c_write(0x72,0x04,0x5A);
	gpio_i2c_write(0x72,0x05,0x03);
	gpio_i2c_write(0x72,0x06,0x0D);
	gpio_i2c_write(0x72,0x07,0x02);

	gpio_i2c_write(0x72,0x63,0x00);	//bit6=1 for DE

	//Color Space
	gpio_i2c_write(0x72,0x08,0x70);
	gpio_i2c_write(0x72,0x09,0x00);
	gpio_i2c_write(0x72,0x0A,0x00);

	gpio_i2c_write(0x72,0x60,0x04);
	...
	//Power Up
	gpio_i2c_write(0x72,0x1E,0x00);

	//Set AVI
	//gpio_i2c_write(0x72,0x0C,14);
#if 1
	gpio_i2c_write(0x72,0x09,0x02);	//  ---Y422 input

	gpio_i2c_write(0x72,0x0A,0x00);	// ---RGB output

	gpio_i2c_write(0x72,0x19,0x00);	// ---enable color convert, must do it here

	gpio_i2c_write(0x72,0x60,0x80);	// ---embeded sync select

	gpio_i2c_write(0x72,0x62,0x58);	// ---HBIT_2_HSYNC L

	gpio_i2c_write(0x72,0x63,0x40);	// ---embede enable and HBIT_2_HSYNC M

	gpio_i2c_write(0x72,0x64,0x4C);	// ---FIELD2_OFST L

	gpio_i2c_write(0x72,0x65,0x04);	// ---FIELD2_OFST M

	gpio_i2c_write(0x72,0x66,0x2C);	// ---HWIDTH L

	gpio_i2c_write(0x72,0x67,0x00);	// ---HWIDTH M

	gpio_i2c_write(0x72,0x68,0x02);	// ---VBIT_2_VSYNC

	gpio_i2c_write(0x72,0x69,0x05);	// ---VWIDTH
#endif


	//Enable TMDS
	gpio_i2c_write(0x72,0x1A,0x00);
